...
function listOverdueDevices(ledger, today) {
  ...
  for (const tag in ledger) {
    ...
    if (device.status === "CheckedOut" && device.dueDate !== "") {
      const due = parseDate(device.dueDate);
      if (compareDates(due, todayDate) < 0) {
        overdue.push(device);
      }
    }
  }
  ...
  overdue.sort((a, b) =>
    compareDates(parseDate(a.dueDate), parseDate(b.dueDate)),
  );
...
// ---------- Quick tests ----------
console.log("--- checkout tag 2 ---");
console.log(
  checkoutDevice(equipmentLedger, "2", {
    name: "Alex Lee",
    email: "alex@acme.org",
  }),
);
